...
export interface BusinessYelp {
  ...
  rating: number;
  location: { address1: string; address2: string; city: string; zip_code: string; country: string; state: string };
  coordinates: {
    latitude: number;
    longitude: number;
  };
  photos: Array<string>;
  price: string;
  hours: Array<{ open: Array<{ is_overnight: boolean; start: string; end: string; day: number }>; hours_type: string }>;
  distance?: number;
}
